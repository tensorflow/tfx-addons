...
class PipelineRunContext(_MetadataClientBase):
  ...
  def get_status(self) -> Dict[str, str]:
    """Retrieve status of component executions for current pipeline run.
        ...
        - 'cached': Component execution skipped to reuse previous
          results from cache.
        Returns:
    ...
    return {
        e.properties[_EXECUTION_TYPE_KEY_COMPONENT_ID].string_value:
        e.properties[_EXECUTION_TYPE_KEY_STATE].string_value
        for e in executions
    }
  ...
  def pipeline_name(self) -> str:
    ...
    return self._context.properties[
        _EXECUTION_TYPE_KEY_PIPELINE_NAME].string_value
  ...
  def pipeline(self) -> "PipelineContext":
    """Retrieve Pipeline context for this pipeline run.
        ...
        """
    context = PipelineContext.get_pipeline_context(self._mlmd,
                                                   self.pipeline_name)
...
class PipelineContext(_MetadataClientBase):
  ...
  def _get_runs(self) -> List[PipelineRunContext]:
    ...
    for context in contexts:
      if (context.properties[_EXECUTION_TYPE_KEY_PIPELINE_NAME].string_value ==
          self.name):
